...
def PerformMinimization():
    ...
    MolCount, ValidMolCount, MinimizationFailedCount = ProcessMolecules(Mols, Writer)
...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    (MolCount, ValidMolCount, MinimizationFailedCount) = [0] * 3
    for Mol in Mols:
        MolCount += 1
        ...
        Mol, CalcStatus, ConfID = MinimizeMoleculeOrConformers(Mol, MolCount)
        
        if not CalcStatus:
            MinimizationFailedCount += 1
            continue
        
        WriteMolecule(Writer, Mol, ConfID)
    
    return (MolCount, ValidMolCount, MinimizationFailedCount)
...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        MolCount += 1
        MolIndex, EncodedMol, CalcStatus, ConfID = Result
...
def WorkerProcess(EncodedMolInfo):
    ...
    if EncodedMol is None:
        return [MolIndex, None, CalcStatus, ConfID]
    ...
    Mol, CalcStatus, ConfID = MinimizeMoleculeOrConformers(Mol, (MolIndex + 1))

    return [MolIndex, RDKitUtil.MolToBase64EncodedMolString(Mol, PropertyPickleFlags = Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps), CalcStatus, ConfID]
...
def MinimizeMoleculeOrConformers(Mol, MolNum = None):
    ...
    ConfID = None
    if OptionsInfo["SkipConformerGeneration"]:
        Mol, CalcStatus = MinimizeMolecule(Mol, MolNum)
    else:
        Mol, CalcStatus, ConfID = MinimizeConformers(Mol, MolNum)

    return (Mol, CalcStatus, ConfID)
...
def MinimizeConformers(Mol, MolNum = None):
    "Generate and mininize conformers for a molecule to get the lowest energy conformer."
    ...
    return (Mol, True, MinEnergyConfID)
...
def WriteMolecule(Writer, Mol, ConfID):
    """Write molecule. """
