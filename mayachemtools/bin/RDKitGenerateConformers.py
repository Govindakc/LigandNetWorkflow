...
def ProcessMolecules(Mols, Writer):
    """Process molecules to generate comformers."""
...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    for Mol in Mols:
        ...
        ConformerMol, CalcStatus, ConfIDs = GenerateMolConformers(Mol, MolCount)
        
        if not CalcStatus:
            ConfGenFailedCount += 1
            continue
        
        WriteMolConformers(Writer, ConformerMol, MolCount, ConfIDs)
...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        MolCount += 1
        MolIndex, EncodedMol, CalcStatus, ConfIDs = Result
        ...
        WriteMolConformers(Writer, Mol, MolCount, ConfIDs)
...
def WorkerProcess(EncodedMolInfo):
    ...
    if EncodedMol is None:
        return [MolIndex, None, CalcStatus, ConfIDs]
    ...
    Mol, CalcStatus, ConfIDs = GenerateMolConformers(Mol, (MolIndex + 1))

    return [MolIndex, RDKitUtil.MolToBase64EncodedMolString(Mol, PropertyPickleFlags = Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps), CalcStatus, ConfIDs]
...
def GenerateMolConformersWithMinimization(Mol, MolNum):
    ...
    return [Mol, True, SelectedConfIDs]
...
def WriteMolConformers(Writer, Mol, MolNum, ConfIDs):
